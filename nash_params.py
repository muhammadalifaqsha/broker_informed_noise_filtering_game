...
def solve_coef_eta(env, bi_nash, theta_B = 10., mu_B = 0., sigma_B = 60., kappa_alpha = 1., sigma_alpha = 1.):
    """
    Solving the coefficients that enter the informed trader's optimal strategy
    """
    ...
    dP = lambda t, p: sigma_B**2 - 2*theta_B*p - (env.b**2)*(p**2)/(env.sigma_s**2)
    p_init = np.array([bi_nash.sigma_0])
    ...
    V_I = sol.y.reshape(-1,)
    V_I_eval = lambda t: CubicSpline(timesteps, V_I)(t)

    _ts = timesteps
    _Gt = lambda t, z: np.array([
                                    # ODE z's
                                    (z[9]*z[0])/(2*bi_nash.k) + mu_B*theta_B*z[2],
                                    1 - kappa_alpha*z[1] + (z[9]*z[1])/(2*bi_nash.k),
                                    env.b - theta_B*z[2] + (z[9]*z[2])/(2*bi_nash.k),
                                    (z[9]*z[0])/(2*bi_nash.k) + (z[0]**2)/(4*bi_nash.k) + mu_B*theta_B*z[5] + env.b* env.corr *sigma_alpha*(1/env.sigma_s)*V_I_eval(env.T-t)*z[6] + (sigma_alpha**2)*z[7] + (env.b*V_I_eval(env.T-t)/env.sigma_s)**2*z[8],
                                    (z[9]*z[1])/(2*bi_nash.k) + (z[0]*z[1])/(2*bi_nash.k) - kappa_alpha*z[4] + mu_B*theta_B*z[6],
                                    (z[9]*z[2])/(2*bi_nash.k) + (z[0]*z[2])/(2*bi_nash.k) - theta_B*z[5] + 2*mu_B*theta_B*z[8],
                                    (z[1]*z[2])/(2*bi_nash.k) - kappa_alpha*z[6] - theta_B*z[6],
                                    (z[1]**2)/(4*bi_nash.k) - 2*kappa_alpha*z[7],
                                    (z[2]**2)/(4*bi_nash.k) - 2*theta_B*z[8],
                                    # ODE for g2
                                    (z[9]**2/bi_nash.k) - (bi_nash.rho0 + bi_nash.rho1*V_I_eval(env.T-t))
                                   ] )
    init_cond = np.zeros((10,))
    init_cond[-1] = -bi_nash.beta0 - bi_nash.beta1*V_I_eval(env.T)
    _sol        = solve_ivp(fun = _Gt, 
                       ...
                       method = 'DOP853')
    z        = _sol.y[:,::-1]
    z_eval = lambda t: CubicSpline(timesteps, z, axis=1)(t)
    
    return z, z_eval
    
def solve_coef_nu(env, bi_nash, theta_B = 10., mu_B = 0., sigma_B = 60., kappa_alpha = 1., sigma_alpha = 1.):
    """
    Solving the coefficients that enter the informed trader's optimal strategy
    """
    timesteps = env.timesteps
    ...
    # computing the coefficents of informed trader's speed with broker's parameter assumptions
...
class broker_informed:
    """
    Broker and informed trader's beliefs on the parameters
    """
    def __init__(self, env, Q0 = 0., k = 1e-3, beta0 = 0, beta1 = 0, rho0 = 0, rho1 = 0,
                 sigma_B_inf = 60., mu_B= 0., theta_B_inf = 10., 
                 sigma_B_br = 50., theta_B_br = 5., kappa_alpha_br = 1.5, sigma_alpha_br = 1.5,
                 sigma_0 = 0.,
                 Q0 = 0, k = 1e-3, beta0 = 0, beta1 = 0, rho0 = 0, rho1 = 0, sigma_B = 60, mu_B = 0, theta_B = 10, sigma_0 = 0, scale_ivp = 1):
        self.Q0 = Q0
        self.k = k
        self.beta0 = beta0
        self.beta1 = beta1
        ...
        self.rho1 = rho1
        self.mu_B = mu_B
        self.sigma_B_inf = sigma_B_inf
        self.theta_B_inf = theta_B_inf
        self.sigma_B_br = sigma_B_br
        self.theta_B_br = theta_B_br
        self.kappa_alpha_br = kappa_alpha_br
        self.sigma_alpha_br = sigma_alpha_br
        self.sigma_0 = sigma_0
        self.timesteps = env.timesteps
        self.N = len(self.timesteps)

        # computing the coefficients in the informed trader's optimal strategy
        
        dP = lambda t, p: self.sigma_B_inf**2 - 2*self.theta_B_inf*p - (env.b**2)*(p**2)/(env.sigma_s**2)
        ...
        self.V_I_inf = sol.y.reshape(-1,)
        self.V_I_inf_eval = lambda t: CubicSpline(self.timesteps, self.V_I_inf)(t)
        
        _ts = self.timesteps
        _Gt_inf = lambda t, z: np.array([
                                        # ODE z's
                                        (z[9]*z[0])/(2*self.k) + self.mu_B*self.theta_B_inf*z[2],
                                        1 - env.kappa_alpha*z[1] + (z[9]*z[1])/(2*self.k),
                                        env.b - self.theta_B_inf*z[2] + (z[9]*z[2])/(2*self.k),
                                        (z[9]*z[0])/(2*self.k) + (z[0]**2)/(4*self.k) + self.mu_B*self.theta_B_inf*z[5] + env.b* env.corr *env.sigma_alpha*(1/env.sigma_s)*self.V_I_eval(env.T-t)*z[6] + (env.sigma_alpha**2)*z[7] + (env.b*self.V_I_eval(env.T-t)/env.sigma_s)**2*z[8],
                                        (z[9]*z[1])/(2*self.k) + (z[0]*z[1])/(2*self.k) - env.kappa_alpha*z[4] + self.mu_B*self.theta_B_inf*z[6],
                                        (z[9]*z[2])/(2*self.k) + (z[0]*z[2])/(2*self.k) - self.theta_B_inf*z[5] + 2*self.mu_B*self.theta_B_inf*z[8],
                                        (z[1]*z[2])/(2*self.k) - env.kappa_alpha*z[6] - self.theta_B_inf*z[6],
                                        (z[1]**2)/(4*self.k) - 2*env.kappa_alpha*z[7],
                                        (z[2]**2)/(4*self.k) - 2*self.theta_B_inf*z[8],
                                        # ODE for g2
